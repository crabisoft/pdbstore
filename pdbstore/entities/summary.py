"""Result of an operation performed against a symbol store."""
...
class Summary:
    ...
    def add_entry(
        ...
        error_msg: Optional[Union[List[str], str]] = None,
        source_stat: Optional[Any] = None,
        **kwargs: Any,
    ) -> Dict[str, Any]:
        """Add new entry result given operation status.

        :param entry: The associated transaction entry.
        :param status: Indicate the operation result.
        :param tr_type: The transaction type.
        :param error_msg: Optional error message.
        :param source_stat: Optional ``stat`` result of the input source file, used to
            report its size and modification time. Reading it is the caller's
            responsibility so that this entity performs no I/O of its own.
        ...
        if status == OpStatus.SUCCESS and tr_type == TransactionType.ADD and source_stat:
            res["mtime"] = datetime.datetime.fromtimestamp(
                source_stat.st_mtime,
                datetime.datetime.now(datetime.timezone.utc).astimezone().tzinfo,
            ).isoformat()
            res["size"] = source_stat.st_size
