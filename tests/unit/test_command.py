...
def fixture_response_file(tmp_path, test_data_dir):
    ...
    with response_path.open("wt", encoding="utf-8") as rpf:
        for root, _, filenames in os.walk(test_data_dir):
            ...
            break
        rpf.write(f"{test_data_dir / 'history'}\n")
    yield response_path
...
def test_build_list_one(test_data_dir):
    """test build_files_list with only one file path"""
    test_path: Path = test_data_dir / "dummyapp.pdb"
